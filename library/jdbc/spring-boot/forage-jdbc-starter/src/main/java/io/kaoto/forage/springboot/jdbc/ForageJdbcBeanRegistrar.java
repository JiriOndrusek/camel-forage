...
class ForageJdbcBeanRegistrar implements ImportBeanDefinitionRegistrar, org.springframework.context.EnvironmentAware {
    ...
    public void registerBeanDefinitions(AnnotationMetadata importingClassMetadata, BeanDefinitionRegistry registry) {
        ...
        } else if (hasDefaultProperties(descriptor)) {
            // No prefixed configurations, but default (unprefixed) properties exist
            LOG.info("Discovered default Forage JDBC configuration, registering default bean");
            String defaultBeanName = descriptor.defaultBeanName();
            if (!registry.containsBeanDefinition(defaultBeanName)) {
                registerBean(registry, descriptor, defaultBeanName, null);
            }
        } else {
            LOG.debug("No Forage JDBC configuration found (neither prefixed nor default)");
        }
    }
    ...
    private boolean hasDefaultProperties(JdbcModuleDescriptor descriptor) {
